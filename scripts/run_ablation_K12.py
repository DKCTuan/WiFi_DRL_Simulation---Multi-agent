...
chạy song song, không đụng file của nhau vì tên file luôn có seed/key):
    ...
    python scripts/run_ablation_K12.py --keys no_qmix,with_vib  --seeds 0
    python scripts/run_ablation_K12.py --keys baseline,no_ddqn,no_qmix,with_vib --seeds 1,2

Output lưu vào:  results/data/ (file .txt training curve + eval summary cho
mỗi ablation) và results/best_models/K12_ablation_<key>/ (checkpoint tốt nhất
...
print(f"  Đã chạy: keys={selected_keys} | seeds={SEEDS} — K=12.")
